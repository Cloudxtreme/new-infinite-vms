...
echo -e "\n" | tee -a /etc/cl.selector/php.conf
echo "Directive = output_buffering" | tee -a /etc/cl.selector/php.conf
echo "Default   = Off" | tee -a /etc/cl.selector/php.conf
echo "Type      = bool" | tee -a /etc/cl.selector/php.conf
echo "Comment   = When output buffering is enabled, PHP sends all of the output to the client only after script processing is complete." | tee -a /etc/cl.selector/php.conf
