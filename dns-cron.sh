...
echo "*/2 * * * * sh /root/infinite-scripts/dns-curl-check.sh > /dev/null 2>&1" | tee -a /var/spool/cron/root
echo "0 */2 * * * sh /root/infinite-scripts/dns-curl-check-bk.sh > /dev/null 2>&1" | tee -a /var/spool/cron/root
