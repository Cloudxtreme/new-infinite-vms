...
#proxys ips
main_one_google="198.251.81.97"
anycast_one_google="198.251.86.35"
...
grep "$backup_one_google" $dns_files && perl -pi -e "s|$backup_one_google|$anycast_one_google|g" $dns_files && sh /root/infinite-scripts/dns-serial-fix.sh && service named reload
fi

#one-equinix
if [ "$first_curl_one_equinix" == "$curl_result" ] && [ "$second_curl_one_equinix" == "$curl_result" ] && [ "$ssl_curl_one_equinix" == "$ssl_curl_result" ]; then
grep "$backup_one_equinix" $dns_files && perl -pi -e "s|$backup_one_equinix|$main_one_equinix|g" $dns_files && sh /root/infinite-scripts/dns-serial-fix.sh && service named reload
fi
#vms - end
